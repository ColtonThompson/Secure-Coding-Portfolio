...
/// Creates a string containing today's date formatted for this assignment - COLTON THOMPSON
std::string get_formatted_date() {
	...
	char buffer[60];
	std::strftime(buffer, 20, "%Y-%m-%d", ltm);
	return buffer;
}
...
void save_data_file(const std::string &filename,
	...
	if (myfile.is_open()) {
		myfile << "Colton Thompson\n";
		myfile << get_formatted_date() << "\n";
		myfile << key << "\n";
		myfile << data;
		myfile.close();
	} else {
...
int main() {
	std::cout << "Date is now " << get_formatted_date() << std::endl;
	...
	std::cout << "Read File: " << file_name << " - Encrypted To: "
			<< encrypted_file_name << " - Decrypted To: " << decrypted_file_name
			<< std::endl;
