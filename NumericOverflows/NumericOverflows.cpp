...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    {
        T future = result + increment;
        // Overflow Check
        if (future != 0 && future >= std::numeric_limits<T>::max()) {
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
        T future = result - decrement;
        // Underflow Check
        if (future != 0 && future <= std::numeric_limits<T>::min()) {
...
void test_overflow()
{
    ...
    // Detected a potential overflow/underflow
